...
func TestReconstructWithIndexes(t *testing.T) {
	...
	// Reconstruct with all shards present
	err = r.ReconstructSpecifyShards(shards)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.ReconstructSpecifyShards(shards, 0, 7, 11)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.ReconstructSpecifyShards(shards, 0, 2)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.ReconstructSpecifyShards(shards, 0, 2, 12)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.ReconstructSpecifyShards(shards, totalShards)
	if err == nil {
